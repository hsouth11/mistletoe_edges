...
dist_seq <- seq(from = 0, to = 45, by = 1)
rolling_quantiles <- map_dfr(dist_seq, function(d){
  ...
  tibble(dist_mid = d + 2.5,
         n = nrow(x2),
         median_sl = median(x2$sl_i_tot_rm, na.rm = TRUE),
         q5_sl = quantile(x2$sl_i_tot_rm, 0.05, na.rm = TRUE),
         q95_sl = quantile(x2$sl_i_tot_rm, 0.95, na.rm = TRUE))
})
...
p1 <- ggplot(x, aes(dist_y_h, sl_i_tot_rm)) +
  ...
  geom_line(data = rolling_quantiles,
            ...
            colour = "#104E8B",
            show.legend = F) +
  scale_linetype_manual(values = c(q5_sl = "dashed",
                                   median_sl = "solid",
                                   q95_sl = "dashed")) +
...
Only include seed load from mature source trees because we didn't incoporate 
seed load from regen-regen pairs in modelling efforts
```{r}
